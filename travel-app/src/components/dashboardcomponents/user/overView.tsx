  ...
  return (
    <div className="space-y-8 p-6 bg-gradient-to-br from-emerald-50 via-white to-emerald-100 rounded-2xl shadow-lg animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-gray-500 text-lg mt-2">{t("dashboard.subtitle")}</p>
      </div>
      ...
      <Card className="border-none shadow-lg hover:shadow-xl transition-all duration-300 bg-white/90 backdrop-blur-md rounded-2xl">
        ...
        <CardContent className="text-sm space-y-1">
          ...
          <p>
            ...
            <span className="font-semibold text-emerald-600">{new Date().toLocaleDateString()}</span>
          </p>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          {
            title: `${pendingTours.length} ${t("dashboard.stats.upcomingTrips")}`,
            subtitle: `${t("dashboard.stats.nextTour")}: ${pendName} Tour`,
            icon: CalendarCheck2,
            color: "from-emerald-400 to-teal-500",
          },
          {
            title: `${reviewLength} ${t("dashboard.stats.reviewsWritten")}`,
            subtitle: `${t("dashboard.stats.averageRating")} 4.6`,
            icon: Star,
            color: "from-blue-400 to-indigo-500",
          },
          {
            title: `${bookfineshed} ${t("dashboard.stats.completedTours")}`,
            subtitle: `${t("dashboard.stats.visitedCountries")} 5 countries`,
            icon: PlaneTakeoff,
            color: "from-yellow-400 to-amber-500",
          },
        ].map(({ title, subtitle, icon: Icon, color }) => (
          <Card key={title} className="border-none shadow-lg hover:shadow-xl transition-all duration-300">
            <CardContent className="flex items-center gap-5 py-6 px-5">
              <div className={`flex items-center justify-center w-14 h-14 rounded-xl bg-gradient-to-br ${color} text-white shadow-md`}>
                <Icon size={28} />
              </div>
              <div>
      ...
      <Card className="shadow-lg rounded-2xl border-none bg-white/90 backdrop-blur">
        <CardHeader>
          <CardTitle className="text-xl font-bold">{t("dashboard.recentBookings")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {allUserBookings?.map((book: any) => (
            <div key={book._id} className="flex justify-between items-center border-b pb-2 last:border-none hover:bg-emerald-50/50 rounded-lg px-2 transition-all">
              <div>
                <p className="font-medium">{book.tour?.name || "Unnamed Tour"}</p>
                <p className="text-sm text-gray-500">{new Date(book.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long" })}</p>
              </div>
              {book.status === "confirmed" ? (
                <Badge className="flex items-center gap-1 bg-green-100 text-green-700 border border-green-300">
                  <CheckCircle className="w-4 h-4" /> {t("dashboard.confirmed")}
                </Badge>
              ) : (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="text-center sm:text-left">
          <p className="text-sm text-gray-500">{t("dashboard.membershipStatus")}</p>
          <p className="font-semibold text-emerald-700">{t("dashboard.plan")}</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button asChild variant="secondary" className="shadow hover:scale-105 transition-transform">
            <Link href="/edituser">{t("dashboard.editProfile")}</Link>
          </Button>
        </div>
      </div>
    </div>
