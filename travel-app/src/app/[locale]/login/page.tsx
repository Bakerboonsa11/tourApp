  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-between bg-gradient-to-b from-green-50 via-green-100 to-green-200 py-10 px-4">
      ...
      <div className="w-full max-w-2xl text-center mb-8">
        <h1 className="text-4xl font-extrabold text-green-700">{t('welcomeTitle')}</h1>
        <p className="text-green-900 text-md mt-2">{t('welcomeSubtitle')}</p>
      </div>
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8 space-y-6 border border-green-100">
        ...
        <h2 className="text-2xl font-bold text-green-700">{t('welcomeBack')}</h2>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">
              {t('noAccount')} <Link href={`/${locale}/signup`} className="text-green-700 hover:underline">{t('signUp')}</Link>
            </span>
            <a href="/forgot-password" className="text-green-700 hover:underline">{t('forgotPassword')}</a>
          </div>
